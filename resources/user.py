...
class UserRegisterResource(Resource) :
    def post(self) :
        ...
        if len(data['password']) < 4 or len(data['password']) > 12 :
            return {'result':'fail','error': '비번 길이가 유효하지 않습니다.'}, 400
        # 4. 비밀번호 암호화 (개인정보보호)
        # 2가지 방법 (원상 복구가능한 암호 -> 양방향 암호)
        # 단방향 암호 hash - 복구가 안됨 - 이것을 사용한다.
        hashed_password = hash_password( data['password'])
        print(hashed_password)
        
        # 5. DB에 이미 회원정보가 있는지 확인한다.
        try:
            connection = get_connection()  #데이터베이스 연결
            query = '''select *
                    from user
                    where email = %s;'''
            record = (data['email'], )
            ...
            if len(result_list) == 1 :
                return {'result':'fail', 'error':'이미 회원가입한 사람'}, 400
            
            # 회원가입이 없으니, 회원가입 코드를 작성한다.
            # DB 저장한다.
            ...
            connection.commit() #데이터에 저장하라 commit 필요
            ### DB에 데이터를 insert 한 후에, 그 insert 된 행의 아이디 가져오는 코드!! 
